...
/-
  **Implementation of Dynamic Epistemic Logic**
-/
...
-- notation `PDL◇`:80 p (act) := λ p, ¬ (PDL□ (¬p) (act)) 
...
-- Constant axioms 
| c₁ { Γ } { t k : const σ } : Proof Γ $ ((c t) ⊃ (c k)) ⊃ c (const.encr t k)
| c₂ { Γ } { t k : const σ } : Proof Γ $ ((c $ const.encr t k) ⊃ (c k)) ⊃ c t
...
structure model := 
  ...
  -- R_B is a serial relation 
  (serial : ∀ w ∈ worlds, ∃ v ∈ worlds, baccess w v = tt)
  (RKB : ∀ w ∈ worlds, ∀ v ∈ worlds, kaccess w v = tt → baccess w v = tt)
